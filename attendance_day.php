...
foreach ($records as $r) {
    ...
    if (empty($r['left_at']) && $isToday) $openCount++;
}
...
require __DIR__ . '/header.php';
?>
<div class="flex flex-wrap items-end justify-between gap-4" data-live-scope="day">
  <div>
    ...
    <p class="mt-1 text-sm text-slate-500">
      ...
      <?php if ($isToday): ?><span class="ml-1 inline-flex items-center gap-1.5 rounded-full bg-emerald-100 px-2 py-0.5 text-xs font-semibold text-emerald-700"><span class="h-1.5 w-1.5 animate-pulse rounded-full bg-emerald-500"></span>live</span><?php endif; ?>
    </p>
  </div>
  <a href="enrollments.php" class="rounded-xl border border-slate-300 bg-white px-4 py-2 text-sm font-semibold text-slate-600 hover:bg-slate-100">👥 Enrollments</a>
</div>
...
<form id="day-filter" method="get" action="attendance_day.php" class="mt-5 grid gap-3 sm:grid-cols-4">
  ...
  <div class="flex items-end">
    <button type="submit" class="w-full rounded-xl bg-indigo-600 px-4 py-2.5 text-sm font-semibold text-white shadow-sm hover:bg-indigo-700">Apply filters</button>
  </div>
</form>
...
<div class="mt-6 grid gap-4 sm:grid-cols-4">
  <?php $stats = [
      ['📥', 'Visits', (string) count($records), 'total'],
      ['🧑‍🎓', 'Students', (string) $uniqueStudents, 'students'],
      ['🟢', 'In course now', (string) ($isToday ? $openCount : 0), 'now'],
      ['⏱', 'Total time', $fmtDur($totalSeconds), 'time'],
  ]; foreach ($stats as $s): ?>
  <div class="rounded-2xl bg-white p-5 shadow-sm ring-1 ring-slate-200">
    <p class="text-2xl"><?= $s[0] ?></p>
    <p class="mt-2 text-3xl font-extrabold text-slate-900" data-day-stat="<?= e((string) $s[3]) ?>"><?= e((string) $s[2]) ?></p>
...
<section class="mt-8">
  <div class="flex items-center justify-between">
    ...
    <?php if ($isToday): ?><span class="text-xs text-slate-400">Durations update live for open sessions</span><?php endif; ?>
  </div>
  <div id="day-records">
  <?php if (!$records): ?>
    <div class="mt-4 rounded-2xl border-2 border-dashed border-slate-300 p-10 text-center text-slate-500">
      <p class="text-4xl">🗓</p><p class="mt-3 font-medium">No attendance recorded for this day.</p>
    </div>
  <?php else: ?>
        ...
        <tr>
          ...
          <td class="px-4 py-3">
            <div class="flex items-center gap-2">
              <span class="grid h-8 w-8 shrink-0 place-items-center rounded-full bg-indigo-600 text-xs font-bold text-white"><?= e(strtoupper(substr((string) $r['name'], 0, 1))) ?></span>
              <div class="min-w-0">
          ...
          <td class="px-4 py-3">
            <span class="inline-block rounded-full bg-indigo-50 px-2 py-0.5 text-xs font-medium text-indigo-700"><?= e((string) $r['course_title']) ?></span>
            <span class="block text-xs text-slate-400"><?= e((string) $r['course_category']) ?></span>
          </td>
          <td class="px-4 py-3">
            <?php if ($open && $isOnline): ?>
              <span class="inline-flex items-center gap-1.5 rounded-full bg-emerald-100 px-2.5 py-1 text-xs font-semibold text-emerald-700"><span class="relative flex h-2 w-2"><span class="absolute inline-flex h-full w-full animate-ping rounded-full bg-emerald-400 opacity-75"></span><span class="relative inline-flex h-2 w-2 rounded-full bg-emerald-500"></span></span> Online</span>
            <?php elseif ($open): ?>
              <span class="inline-flex items-center gap-1.5 rounded-full bg-amber-100 px-2.5 py-1 text-xs font-semibold text-amber-700">⏳ In course</span>
            <?php else: ?>
              <span class="inline-flex items-center gap-1.5 rounded-full bg-slate-100 px-2.5 py-1 text-xs font-semibold text-slate-600">⏹ Left <?= date('g:i A', $endTs) ?></span>
            <?php endif; ?>
          </td>
          <td class="px-4 py-3 text-slate-600">
            <?php if ($open): ?>
              <span data-open-seconds="<?= max(0, $now - (int) $r['entered_at']) ?>" class="font-semibold text-emerald-700"></span>
            <?php else: ?>
              <?= $fmtDur($dur) ?>
            <?php endif; ?>
          </td>
  ...
  <?php endif; ?>
  </div><!-- /#day-records (live-refreshed by app.js) -->
</section>
